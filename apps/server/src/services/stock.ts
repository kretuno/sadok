...
export type StockTransaction = TxDatabase;
...
function recalculateProductWacSync(
  tx: TxDatabase,
  productId: number,
  reason: string,
  referenceId?: number
...
interface AdjustProductStockInput {
  productId: number;
  quantity: number;
  reason: string;
  userId?: number;
...
export function deductProductStock(tx: TxDatabase, input: AdjustProductStockInput) {
  ...
  recalculateProductWacSync(tx, input.productId, input.menuId ? 'menu_confirmation' : 'manual_adjustment', input.menuId);
}

export async function adjustProductStock(input: AdjustProductStockInput) {
  db.transaction((tx) => deductProductStock(tx, input));
...
export function restoreMenuStockDeductions(
  ...
  for (const productId of touchedProducts) {
    recalculateProductWacSync(tx, productId, 'menu_cancel_return', input.menuId);
  }
}
